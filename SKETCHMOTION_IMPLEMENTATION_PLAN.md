...
| 2026-07-14 | Wave 4 | Completed release hardening: project imports reject malformed, empty, and unsupported-version files without replacing the current document; version 2–4 imports migrate through the normalized ProjectV5 path. Added accessibility, migration, and malformed-file browser coverage. | Focused Chromium hardening suite: 3 passed, including zero serious/critical axe violations; final GitHub Actions matrix passed all 87 Chromium, Firefox, and WebKit tests. | Complete |
| 2026-07-14 | Wave 4 | Completed browser release verification and documented local development, build, portable build, and browser-matrix commands. | `npm run build`; `npm run build:portable`; local Chromium Playwright suite: 29 passed; final GitHub Actions matrix: 87 passed across Chromium, Firefox, and WebKit. | Complete |
...
| 2026-07-14 | UI correction | Restored the drawing-layer labels to Foreground and Background, matching the product’s plain-language mental model while retaining existing layer behavior. | Focused Chromium layer/inspector regressions: 2 passed; production build passed. | Complete |
| 2026-07-14 | UI correction | Moved the colour controls directly beneath Brush Settings so primary drawing controls remain together and immediately reachable. | Production build passed; final cross-browser GitHub Actions matrix passed. | Complete |
| 2026-07-14 | UI correction | Removed the redundant contextual “Brush / active layer” inspector blurb to reclaim vertical space, while retaining the compact Brush Settings heading. | Focused navigation and Hand-tool regressions passed; production build passed. | Complete |
| 2026-07-14 | CI hardening | Corrected grouped-alignment assertions, made the WebM download assertion capability-aware in WebKit, and removed stale inspector assertions after the intentional UI removal. | Focused affected tests passed in Chromium, Firefox, and WebKit; final GitHub Actions matrix: 87 passed. | Complete |
| 2026-07-14 | Release | Merged release PR #1 and CI hotfix PR #2 into `main`, then published the production artifact to the existing GitHub Pages URL. | GitHub Actions run `29312120014`: verify and deploy succeeded; live HTML confirmed Foreground, Background, Brush Settings, and removal of the redundant inspector blurb. | Complete |
